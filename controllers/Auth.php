...
Class Auth extends CI_Controller
{
    ...
    */
    ...
    public function login()
    {
        ...
        //info_log("auth", "_SESSION['usr_no'] = [" . $_SESSION['usr_no'] . "]");
        //info_log("auth", "usr_no = [" . $usr_no . "]");
        //info_log("auth", "idntfr = [" . $idntfr . "]");
        //info_log("auth", "prcs_cls = [" . $prcs_cls . "]");

        if (!isset($_SESSION['usr_no']) && strlen($usr_no) > 0 && strlen($idntfr) > 0 && strncmp($prcs_cls, "r", 1) !== 0)
        {
            info_log("auth/login", "Autologin Begin!");
            auto_login();
        }
        ...
        if ($_POST)
        {
            ...
            if ($this->form_validation->run() == TRUE)
            {
                ...
                if (password_verify($pswd , $usr_info->pswd))
                {
                    ...
                    $result = $this->stay_m->delete_tba001i01_2($usr_no);

                    if ($result)
                    {
                        info_log("auth/login/delete_tba001i01_2", "보관기간 초과 자동로그인정보 삭제 완료");
                    }
                    else
                    {
                        info_log("auth/login/delete_tba001i01_2", "last_query  = [" . $this->db->last_query() . "]");
                        $this->db->trans_rollback();
                        alert_log("auth/login/delete_tba001i01_2", "[SQL ERR] 보관기한 초과 자동로그인정보 삭제 오류!");
                    }
                    ...
                    {
                        //info_log("auth/login", "Autologin Setting Begin!");
                        ...
                        $result = $this->stay_m->insert_tba001i01($i_data);

                        if ($result)
                        {
                            info_log("auth/login/insert_tba001i01", "자동 로그인 정보 생성 완료");
                            ...
                            $result = $this->stay_m->insert_tba001i03($i_data);

                            if ($result)
                            {
                                $this->db->trans_commit();
                                info_log("auth/login/insert_tba001i03", "자동로그인 이력 생성 완료");
                            }
                            else
                            {
                                info_log("auth/login/insert_tba001i03", "last_query  = [" . $this->db->last_query() . "]");
                                $this->db->trans_rollback();
                                alert_log("auth/login/insert_tba001i03", "[SQL ERR] 자동로그인 이력 생성 오류!");
                            }
                            ...
                            info_log("auth/login", "자동로그인 처리 정상 완료!");
                        ...
                        {
                            info_log("auth/login/insert_tba001i01", "last_query  = [" . $this->db->last_query() . "]");
                            $this->db->trans_rollback();
                            alert_log("auth/login/insert_tba001i01", "[SQL ERR] 자동로그인정보 입력 오류!");
                        }
                    ...
                    else
                    {
                        ...
                        {
                            $this->db->trans_commit();
                            info_log("auth/login/insert_tba001i03", "로그인 이력 생성 완료");
                        }
                        else
                        {
                            info_log("auth/login/insert_tba001i03", "last_query  = [" . $this->db->last_query() . "]");
                    ...
                    info_log("auth/login", "사용자 " . $usr_no . " 로그인 되었습니다!!!");
                    info_log("auth/login", "로그인 완료!");
                    info_log("auth/login", "================================================================================");
    ...
    public function logout()
    {
        ...
        info_log("auth/logout", "================================================================================");
        info_log("auth/logout", "로그아웃 시작!");

        $usr_no = get_cookie('usr_no');
        $idntfr = get_cookie('idntfr');

        //info_log("auth/logout","usr_no = [" . $usr_no . "]");
        //info_log("auth/logout","idntfr = [" . $idntfr . "]");
        ...
        if (isset($idntfr))
        {
            ...
            $result = $this->stay_m->delete_tba001i01_1($d_data);
            ...
            if ($result)
            {
                ...
                if ($prcs_cnt == 0)
                {
                    info_log("auth/logout/delete_tba001i01_1", "last_query  = [" . $this->db->last_query() . "]");
                    $this->db->trans_rollback();
                    info_log("auth/logout/delete_tba001i01_1", "[SQL ERR] 자동 로그인 정보 삭제 건수 오류[" . $prcs_cnt . "]!");
                }
                else
                {
                    $this->db->trans_commit();
                    info_log("auth/logout", $usr_no . " 자동 로그인 정보 삭제 완료!");
            ...
            else
            {
                info_log("auth/logout/delete_tba001i01_1", "last_query  = [" . $this->db->last_query() . "]");
                $this->db->trans_rollback();
                alert_log("auth/logout/delete_tba001i01_1", "[SQL ERR] 자동 로그인 정보 삭제 오류!");
            }
        }
        ...
        alert_log("auth", "로그아웃 되었습니다!!!", base_url("auth"));
        info_log("auth/logout", "로그아웃 완료!");
        info_log("auth/logout", "================================================================================");
    }
