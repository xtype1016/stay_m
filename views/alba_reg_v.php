    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <?php echo form_open('', 'class="form-horizontal" id="expns_reg_form"'); ?>
                <div>
                    <h5><strong>아이들 알바 입출금 등록/수정</h5></strong>
                </div>
                ...
                <div class="form-group form-group-sm row">
                    <label class="col-xs-4 control-label" for="io_cls"><h6><strong>입출금 분류</h6></strong></label>
                    <div class="col-xs-8">
                ...
                <div class="form-group form-group-sm row">
                    <label class="col-xs-4 control-label" for="emp_no"><h6><strong>작업자</h6></strong></label>
                    <div class="col-xs-8">
                        <select class="form-control" id="emp_no" name="emp_no">
                            <?php foreach($emp_list as $e_list) { if (isset($view) && strcmp($view->emp_no, $e_list->clm_val) == 0) { $selected = TRUE; } else { $selected = FALSE; } ?>
                                <option value="<?php echo $e_list->clm_val; ?>" <?php echo set_select('emp_no', $e_list->clm_val, $selected)?>><?php echo $e_list->clm_val_nm; ?></option>
                            <?php }; ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group form-group-sm row">
                    <div class="pull-right">
                        <?php if (strncmp($prcs_cls, 'i', 1) == 0) { ?>
                            <button type="submit" class="btn btn-primary btn-sm" id="ins" formaction="/alba/ins">입력</button>
                            <button type="submit" class="btn btn-primary btn-sm" id="ins_r" formaction="/alba/ins/r">계속 입력</button>
                        <?php } else if (strncmp($prcs_cls, 'u', 1) == 0) { ?>
                            <button type="submit" class="btn btn-primary btn-sm" id="upd" formaction="<?php echo '/alba/upd/' . $this->uri->segment(3); ?>">수정</button>
                            <button type="submit" class="btn btn-warning btn-sm" id="del" formaction="<?php echo '/alba/del/' . $this->uri->segment(3); ?>">삭제</button>
                        <?php } ?>
                    </div>
                </div>
            </form>
